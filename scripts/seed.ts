...
async function main() {
  ...
  const barber2 = await prisma.barber.create({
    data: {
      ...
      bio: 'Experto en diseño de barba y afeitado tradicional. Combino técnicas clásicas con tendencias modernas para un look impecable. Cada servicio es una experiencia de relajación y estilo.',
      specialties: 'Diseño de barba, Afeitado clásico, Texturas',
      hourlyRate: 30,
      isActive: true,
  ...
  const barber4 = await prisma.barber.create({
    data: {
      ...
      bio: 'Veterano en el arte de la barbería con enfoque en la atención al detalle. Cada servicio es una experiencia premium. Más de 15 años perfeccionando mi técnica para ofrecer resultados excepcionales.',
      specialties: 'Cortes ejecutivos, Tratamientos capilares, Consultoría de estilo',
      hourlyRate: 35,
      isActive: true,
  ...
  const services = [
    ...
    // Barber 2 Services
    {
      ...
      description: 'Perfilado y diseño profesional de barba con navaja caliente',
      duration: 30,
      price: 30,
      barberId: barber2.id,
    },
