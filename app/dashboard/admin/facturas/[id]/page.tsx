  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="print:hidden fixed top-4 right-4 z-50 flex gap-2">
        <Button onClick={() => router.push('/dashboard/admin/facturas')} variant="outline">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Volver
        </Button>
        <Button onClick={handlePrint} className="bg-[#00f0ff] text-black hover:bg-[#00d0df]">
              ...
              <tbody>
                {invoice.items.map((item: any, index: number) => (
                  <tr key={index} className="border-b border-gray-200">
                    ...
                    <td className="text-right p-3 text-gray-700">${item.unitPrice.toFixed(2)}</td>
                    <td className="text-right p-3 text-gray-700">${item.total.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-end">
            <div className="w-64">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-gray-900 font-semibold">${invoice.amount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between pt-3 border-t border-gray-300">
                <span className="text-xl font-bold text-gray-900">Total:</span>
                <span className="text-2xl font-bold text-[#00f0ff]">${invoice.amount.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
